...
class DbRecordMaker():
    ...
    def aec_score_schema():
        columns = ["asin", "timestamp", "overall", "score_dist_amz", "fiability", "value", "pros", "cons"]
        dtypes = [str, str, float, np.array, float, float, str, str]
        return {k: v for k, v in zip(columns, dtypes)}

    @staticmethod
    def _reviews_schema():
        columns = ["review_id", "timestamp", "asin", "title", "body", "link",
    ...
    @staticmethod
    def product_schema():
        table_columns = ["asin", "timestamp", "product_name", "title",
                         ...
                         "similar_products", "affiliate_link"]
    ...
    def _get_product_record(self):
        ...
        affiliate_link = self.amazon_affiliate_link(self.p.asin)

        record = (self.p.asin, timestamp,  prod_name, self.p.title,
                  ...
                  self.p.similar_products, affiliate_link)
    ...
    def _aec_score_record(self):
        ...
        pros = None
        cons = None
        return (self.p.asin, self.timestamp, overall, score_dist_amz, fiability, value, pros, cons)
    ...
    def amazon_affiliate_link(self, asin):
        """
        Returns the amazon affiliate link for a given ASIN
        :param asin:
        :return:
        """
